...
class ScaleMesh extends TransformMesh {
    ...
    handleUniformTransform({sceneNode, intersectionDelta, intersectionPoint}) {
        super.handleTransform({sceneNode, intersectionDelta, intersectionPoint});
        handleScaling({sceneNode, intersectionDelta, intersectionPoint}, ['x', 'y', 'z'], this._scaleFactor);
    }
}
class ScaleXMesh extends ScaleMesh {
    ...
    handleTransform({sceneNode, intersectionDelta, intersectionPoint}) {
        if (CurrentTransformOrientation.isGlobal()) {
            super.handleUniformTransform({sceneNode, intersectionDelta, intersectionPoint});
        } else if (CurrentTransformOrientation.isLocal()) {
            super.handleTransform({sceneNode, intersectionDelta, intersectionPoint});
            handleScaling({sceneNode, intersectionDelta, intersectionPoint}, ['x'], this._scaleFactor);
        }
    }
}
class ScaleYMesh extends ScaleMesh {
    ...
    handleTransform({sceneNode, intersectionDelta, intersectionPoint}) {
        if (CurrentTransformOrientation.isGlobal()) {
            ...
            super.handleTransform({sceneNode, intersectionDelta, intersectionPoint});
            handleScaling({sceneNode, intersectionDelta, intersectionPoint}, ['y'], this._scaleFactor);
        }
    }
}
class ScaleZMesh extends ScaleMesh {
    ...
    handleTransform({sceneNode, intersectionDelta, intersectionPoint}) {
        if (CurrentTransformOrientation.isGlobal()) {
            ...
            super.handleTransform({sceneNode, intersectionDelta, intersectionPoint});
            handleScaling({sceneNode, intersectionDelta, intersectionPoint}, ['z'], this._scaleFactor);
        }
    }
}

function handleScaling({sceneNode, intersectionDelta, intersectionPoint}, axes, scalingFactor) {
    const baseWorldPosition = mat4.getTranslation(vec3.create(), sceneNode.worldMatrix);
    ...
    sceneNode.applyScaling(mat4.fromScaling(mat4.create(), scale));
}
