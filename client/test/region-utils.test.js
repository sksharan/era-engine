...
describe('region-utils#getVertices', function() {
    ...
    it('returns correct data for adjacent tiles not at the origin', function() {
        let tile1 = { loc: { x: 4, y: 0, z: 4 } };
        let tile2 = { loc: { x: 5, y: 0, z: 4 } };
        let hexRadius = 10;
...
function assertCorrectVertices(tile, hexRadius, data, indexOffset) {
    let expected = getExpectedVertices(tile.loc.x, tile.loc.z, hexRadius);
...
function getExpectedVertices(locX, locZ, hexRadius) {
    const centerX = locX * hexRadius * 1.5;
    const centerZ = locZ * hexRadius * 0.866 * 2;
