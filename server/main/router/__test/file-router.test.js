...
describe('File router', () => {
    ...
    it('should allow fetching metadata of uploaded files', async () => {
        ...
        const res1 = await request(app).get(`${FileRouterEndpoint}/${files[1]._id}/metadata`).expect(200);
    ...
    it('should return bad request if trying to fetch metadata with an invalid id', async () => {
        await request(app).get(`${FileRouterEndpoint}/333333333333/metadata`).expect(400);
    });

    it('should return success when downloading uploaded files', async () => {
        const files = await uploadFiles();
        await request(app).get(`${FileRouterEndpoint}/${files[0]._id}/content`).expect(200);
        await request(app).get(`${FileRouterEndpoint}/${files[1]._id}/content`).expect(200);
    });

    it('should return bad request if trying to fetch content with an invalid id', async () => {
        await request(app).get(`${FileRouterEndpoint}/333333333333/content`).expect(400)
    });
});
